...
/// Loads and validates a DataFrame with required schemas enforcement
fn load_and_validate_engagement(path: &str) -> Result<DataFrame> {
    let mut df = load_dataframe(path)?;

    // Define an expected schemas and cast to it
    df = df
        .lazy()
        .with_columns([
        ...
        .collect()
        .context("Failed to cast engagement data to expected schemas")?;

    Ok(df)
}

/// Loads and validates metadata with schemas enforcement
fn load_and_validate_metadata(path: &str) -> Result<DataFrame> {
    ...
    df = df
        ...
        .collect()
        .context("Failed to cast metadata to expected schemas")?;
...
pub fn build_matrices(
    ...
    // Contextual features (like a device) are handled by
    ...
    for i in 0..df_meta.height() {
        let item_id = match items_meta.get(i) {
            Some(id) if !id.is_empty() => id,
            _ => continue, // Skip a row if item_id is null or empty
        };
...
/// Reads a Parquet or CSV file from the disk into a Polars DataFrame.
...
fn add_user_feature(
    ...
    if let Some(val) = value {
        if !val.is_empty() && val != "null" {
            ...
            // Fix for the borrow checker: get len *before* entry call
...
/// Converts tenure from days (integer) to a string-based bucket.
